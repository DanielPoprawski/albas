...
has the module layout and known gaps. Account data is disposable — never write data migrations.
...
- **Synced column = 3 touches**: schema, `src-tauri/src/sync.rs` `TABLES`, `sharedLogic.ts` (+
  `persistence.ts` both branches and `src/ipc.ts`). Mismatch = silent no-sync. `todos.category` /
